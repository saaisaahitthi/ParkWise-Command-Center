...
export const mockSimulatorPresets = [
  ...
  },
  {
    ...
    overrides: { total_officers: 25, critical_min_officers: 4, high_min_officers: 2 },
  },
];
...
export function getMockResponse<T>(url: string, _params?: unknown): T {
  ...
  if (cleanUrl === "simulator/presets") return mockSimulatorPresets as unknown as T;
  if (cleanUrl === "simulator/baseline") return mockSimulatorBaseline as unknown as T;
...
export function getMockPostResponse<T>(url: string, body?: unknown): T {
  ...
  if (cleanUrl === "simulator/run") {
    const officers =
      (payload?.overrides as Record<string, number> | undefined)?.total_officers ?? 20;
    const avgEis = Math.max(45, 95 - officers * 1.2);
    return {
      scenario_name: (payload?.scenario_name as string) ?? "Mock scenario",
      total_hotspots: 4,
      improved_hotspots: 2,
      worsened_hotspots: 0,
      unchanged_hotspots: 2,
      baseline_average_eis: 77.5,
      simulated_average_eis: Number(avgEis.toFixed(1)),
      average_eis_delta: Number((avgEis - 77.5).toFixed(1)),
      baseline_total_officers: 20,
      simulated_total_officers: officers,
      hotspot_results: [{ impact_notes: ["Mock simulation applied."] }],
      summary: { officers_added: Math.max(0, officers - 20) },
    } as unknown as T;
  }
